...
public class RoomNotificationRepository : DynamoRepository, IRoomNotificationRepository
{
    ...
    {
        return await GetAllAsync<RoomNotificationEntity>($"roomNotification#{userId}", cancellationToken);
    }
    ...
    {
        return await GetAsync<RoomNotificationEntity>($"roomNotification#{userId}", roomId, cancellationToken);
    }
    ...
    {
        return await SaveAsync(roomNotification, cancellationToken);
    }
    ...
    {
        return await DeleteAsync($"roomNotification#{userId}", roomId, cancellationToken);
    }
    ...
    {
        return await BatchGetAsync(userIds.Select(q => new RoomNotificationEntity
        {
            UserId = q,
            RoomId = roomId
    ...
    {
        await BatchWriteAsync(new List<IEntity>(roomNotification), new List<IEntity>(), cancellationToken);
        return true;
    }
}